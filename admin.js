...
onSnapshot(lootQuery, snapshot=>{
    ...
    snapshot.forEach(async docSnap=>{
        ...
        // Loot items with edit price & settle buttons
        ...
        entry.loot.forEach((item,index)=>{
            ...
            // Per-loot settle button
            const settleBtn = document.createElement('button');
            settleBtn.textContent=item.settled?'Settled':'Settle';
            settleBtn.className=item.settled?'bg-gray-400 text-white px-2 py-1 rounded ml-1':'bg-green-600 text-white px-2 py-1 rounded ml-1';
