...
addBossBtn.addEventListener('click', async ()=>{
    ...
    bosses.push(name); newBossInput.value='';
});
...
addMemberBtn.addEventListener('click', async ()=>{
    ...
    members.push(name); newMemberInput.value='';
});
...
function renderLootItems(){
    ...
    lootItems.forEach((item,idx)=>{
        const li = document.createElement('li'); li.className='flex justify-between items-center p-1';
        ...
        settleBtn.textContent = item.settled?'Settled':'Settle';
        settleBtn.className = item.settled?'bg-gray-400 text-white px-2 py-1 rounded':'bg-green-600 text-white px-2 py-1 rounded';
        settleBtn.onclick = ()=>{ lootItems[idx].settled=true; renderLootItems(); };
        li.appendChild(settleBtn);

        // Remove button
        const removeBtn = document.createElement('button');
        removeBtn.textContent='Remove'; removeBtn.className='bg-red-500 text-white px-2 py-1 rounded ml-1';
...
saveBossBtn.addEventListener('click', ()=>{
    ...
    if(selected){ selectedBoss = selected.value; selectedBossSpan.textContent=selectedBoss; }
...
onSnapshot(lootQuery, snapshot=>{
    ...
    snapshot.forEach(async docSnap=>{
        const entry = docSnap.data(); const entryId = docSnap.id;
        ...
        entry.members.forEach(m=>{const li=document.createElement('li');li.textContent=m; ulMembers.appendChild(li);});
        membersDiv.appendChild(ulMembers); membersDiv.className='mb-2'; expandedDiv.appendChild(membersDiv);

        // Loot items with per-loot settle button & edit price
        ...
        entry.loot.forEach((item,index)=>{
            ...
            // Edit price
            const priceBtn = document.createElement('button'); priceBtn.className='bg-indigo-200 px-2 py-1 rounded';
            ...
            // Per-loot settle
            const settleBtn = document.createElement('button'); settleBtn.textContent=item.settled?'Settled':'Settle';
            settleBtn.className=item.settled?'bg-gray-400 text-white px-2 py-1 rounded':'bg-green-600 text-white px-2 py-1 rounded ml-1';
        ...
        entry.loot.forEach(l=>{
            ...
            const li = document.createElement('li'); li.textContent=`${l.name}: ${share} per member`; ulShares.appendChild(li);
        });
        ...
        // Remove Entry Button
        const removeBtn=document.createElement('button');
        removeBtn.textContent='Remove'; removeBtn.className='bg-red-500 text-white px-2 py-1 rounded mt-1';
